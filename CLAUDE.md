...
**Service alerts** are decoded with the library's **private** GTFS Realtime bindings in `internal/gtfsrt` into the library-owned `Alert` type rather than exposing the protobuf types. Don't switch back to a public bindings module: the Go protobuf runtime panics at init when two packages register the same proto names, so any program that also linked another copy of the GTFS-rt bindings (package `transit_realtime`) would fail to start. The private copy of `gtfs-realtime.proto` renames only the proto package, `go_package` and file name (see its header; `TestNoSharedRegistryNames` guards this). To update it, copy the new upstream file from google/transit, reapply those renames, and run `go generate ./internal/gtfsrt` (needs `protoc` and `protoc-gen-go`). NJ Transit only populates the deprecated `active_period`, not `communication_period`/`impact_period`. NJTransit sends `cause`/`effect` as `UNKNOWN_*` only, informed entities carry GTFS `route_id`, `stop_id` or a `trip` (whose `route_id` fills `AlertEntity.RouteID`), and alert text has `?` where an en dash was; the text is left as sent.
